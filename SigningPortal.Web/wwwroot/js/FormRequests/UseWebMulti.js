...
function customImageParser(block) {
    if (block.data) {
        ...
        const percentageNumber = parseFloat(block.data.height.replace('%', ''));
        const decimalValue = percentageNumber / 100;
...
async function saveAnnotations() {
    ...
    const signele = document.getElementById(`Signature_${userrole}`)
    if (signele) {
        overlaysignflag = true;
    }
    const esealele = document.getElementById(`Eseal_${userrole}`)
    if (esealele) {
            ...
            if (response.status == "Success") {
                swal({
                    ...
                    if (isConfirm) {
                        ...
                        window.location.href = FormRequestsIndexUrl + '?viewName=' + flag1;
