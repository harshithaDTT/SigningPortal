...
$(document).ready(function () {
    ...
    if ($.fn.DataTable.isDataTable('#ReceivedTb')) {
        $('#ReceivedTb').DataTable().destroy(); // Destroy the existing instance
        $('#ReceivedTb').empty();
    }

    var table = $('#ReceivedTb').DataTable({
        searching: false,
        serverSide: true,
        paging: false,
        ordering: false,
            ...
            function fetchDocuments() {
                ...
                currentSentAjaxRequest = $.ajax({
                    ...
                    success: function (res) {
                        ...
                        //      if ($('#receivedCardsContainer').length) {
                        //             $('#receivedCardsContainer').empty();
                        //      }
                        // }
                        ...
                        if (res.data.length === 0 && currentPage === 0) {
                            ...
                            if ((currentPage + 1) * pageSize >= totalRecords) {
                                ...
                                if (totalRecords > pageSize) {
                                    $('#allDocsLoadedMsgRec').show();
                                    document.getElementById("allDocsLoadedMsgRec").innerHTML = `All documents loaded (${totalRecords})`
                                }
    ...
    $('#showMoreBtnRec').on('click', function () {
        currentPage++;
        $('#showMoreBtnRec').hide();
    ...
    $('#searchButton').on('click', function () {
        ...
        $('#noDataMessageRec').hide();
        $('#showMoreBtnRec').hide();
        $('#ReceivedTb').DataTable().ajax.reload();
    });


    $(document).on("click", ".checkboxContainer", function (event) {
        event.stopPropagation(); // same as in your inline code
...
function getstatstics() {
    ...
    $.ajax({
        url: GetReceiveddocumentsStatstics,
        type: 'GET',
        dataType: 'json',
...
async function renderAndAppendCards(data, total) {
    let container = $('#receivedCardsContainer');
...
function showShimmer(count) {
    ...
    $('#receivedCardsContainer').append($shimmer);
...
//    return `
//                              <div id="doc-${current}" class="card mb-3 shadow-sm border rounded-3 p-1 pl-2 pr-2 onhoverclass redirect-doc" style="cursor:pointer;" data-id="${id}" >
...
//                                                 ${documentName}
//                                             </h5>
//                                             <p class="sent-by p-0 m-0">From: <span class="sender-badge">${ownerName}</span></p>
...
function renderActionButtonHtml(item, suid, userEmail, currentDocNo, totalDocs) {
    ...
    const style = "font-size:small;padding:4px 12px;max-width:90px;width:80px;text-transform: none !important;";
    const id = item.id || item._id; // in case it's still _id
    const recipient = (item.recepients || []).find(r => r.suid === suid);
    const pendingSignList = item.pendingSignList || [];
    const completeSignList = item.completeSignList || [];
    const firstUser = pendingSignList[0];

    // Helper: return view button
    const viewBtn = `<button type='button' class='${baseBtn} redirect-doc' style='${style}' data-id="${id}">
                                <i class='fa-regular fa-eye'></i>&nbsp;View</button>`;

    // Helper: return sign button
    const signBtn = `<button type='button' class='${baseBtn} sign-button' style='${style}' data-id='${id}' data-current='${currentDocNo}' data-total='${totalDocs}'>
                                <i class='fa fa-pencil'></i>&nbsp;Sign</button>`;

    // Helper: return retry button
    const retryBtn = `<button type='button' class='${baseBtn} sign-button' style='${style}' data-id='${id}' data-current='${currentDocNo}' data-total='${totalDocs}'>
                                <i class='fa-solid fa-arrow-rotate-left'></i>&nbsp;Retry</button>`;

    if (item.status === "In Progress") {
        if (!item.disableOrder) {
            if (firstUser && firstUser.suid === suid && firstUser.email === userEmail) {
                if (!recipient) return viewBtn;

                if (recipient.hasDelegation) return viewBtn;

                if (["Failed", "Pin_Failed"].includes(recipient.status)) {
                    return retryBtn;
                } else if (["Signing_in_Progress", "Signing_Pending", "Waiting_for_Others", "Signed", "Rejected"].includes(recipient.status)) {
                    return viewBtn;
                } else {
                    return signBtn;
                }
            ...
            const hasSigned = completeSignList.some(r => r.suid === suid);
            ...
            if (hasSigned) {
                ...
                if (["Failed", "Pin_Failed"].includes(recipient.status) && recipient.signedBy === userEmail) {
                    return retryBtn;
                } else if (["Signing_in_Progress", "Signing_Pending", "Waiting_for_Others", "Signed", "Rejected"].includes(recipient.status)) {
                    return viewBtn;
                } else {
    ...
    return viewBtn; // fallback for non "In Progress" status
}