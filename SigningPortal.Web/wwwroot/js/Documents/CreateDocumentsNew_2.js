...
function getMandatoryRecipientCount() {
    ...
    return window.recipients.filter(r => r.mandatory !== false).length;
}

/**
 * Updates the minimum signatures display with auto-calculated value
 */
function updateMinSignaturesDisplay() {
    const totalCount = getRecipientCount();
    const mandatoryCount = getMandatoryRecipientCount();

    $('#requiredSignaturesCount').text(mandatoryCount);
    $('#totalRecipientsCount').text(totalCount);
    $('#RequiredSignatureInput').val(mandatoryCount > 0 ? mandatoryCount : 1);

    // Update legacy hidden field for backend compatibility
    $('#AllSignatoriesRequired').prop('checked', mandatoryCount === totalCount);
}
 ...
 * - Recipient badge updates with count
 */
function updateWorkflowMode() {
    ...
    const recipientBadge = $('#recipientCount');
    ...
    if (recipientBadge.length) {
        recipientBadge.text(recipientCount);
    }
    ...
    // Auto-calculate minimum signatures based on mandatory recipients
...
function getRecipientCount() {
    ...
    return getEstimatedRecipientCount();
}
...
function getEstimatedRecipientCount() {
    ...
    if (typeof recipientSelectors === 'object' && recipientSelectors !== null) {
        if (recipientSelectors.list && recipientSelectors.card) {
            const $cards = $(recipientSelectors.list).find(recipientSelectors.card);
            if ($cards.length) {
                return $cards.length;
            }
        }
        if (recipientSelectors.legacyRow) {
            const $legacyRows = $(recipientSelectors.legacyRow).filter(function () {
                return !$(this).hasClass('classshide');
            });
    ...
    if (Array.isArray(PerpareDocumentContext.receipientEmails)) {
        return PerpareDocumentContext.receipientEmails.length;
    }
...
function getMandatorySignatureCountEstimate(totalRecipients) {
    ...
    if (Array.isArray(window.modalobjlist) && window.modalobjlist.length) {
        mandatoryCount = window.modalobjlist.reduce(function (count, entry) {
            return count + (entry && entry.signatureMandatory ? 1 : 0);
        }, 0);
    } else if (Array.isArray(window.recipients) && window.recipients.length) {
        mandatoryCount = window.recipients.reduce(function (count, rec) {
            if (!rec) {
                return count;
            }
            return count + (rec.mandatory === false ? 0 : 1);
        }, 0);
    }

    if (mandatoryCount === 0) {
        if ($('#AllSignatoriesRequired').prop('checked')) {
...
function updateRequiredSignatureState(options) {
    ...
    const $wrapper = $(REQUIRED_SIGNATURE_SELECTORS.wrapper);
    const $input = $(REQUIRED_SIGNATURE_SELECTORS.input);
    const $error = $(REQUIRED_SIGNATURE_SELECTORS.error);
    ...
    const allRequired = $('#AllSignatoriesRequired').prop('checked');
    const shouldShow = multiEnabled && !allRequired;
    const recipientCount = getEstimatedRecipientCount();
    const baseCount = recipientCount > 0 ? recipientCount : 1;
    const mandatoryCount = shouldShow ? Math.max(1, getMandatorySignatureCountEstimate(recipientCount)) : 0;

    if ($wrapper.length) {
        $wrapper.css('display', shouldShow ? 'flex' : 'none');
        $wrapper.attr('aria-hidden', (!shouldShow).toString());
    }
    ...
    if (!shouldShow) {
        ...
        const storedValue = parseInt($input.val(), 10);
        const derived = Number.isNaN(storedValue) ? baseCount : storedValue;
        numberofsignatures = Math.max(1, derived);
        if (typeof PerpareDocumentContext === 'object' && PerpareDocumentContext !== null) {
            PerpareDocumentContext.signaturesRequiredCount = numberofsignatures;
        }
        return {
    ...
    let parsed = parseInt($input.val(), 10);
    if (Number.isNaN(parsed) || parsed < 1) {
        parsed = mandatoryCount || 1;
    }

    const minValue = Math.max(1, mandatoryCount || 1);
    const maxValue = Math.max(minValue, baseCount);
    const clamped = Math.min(Math.max(parsed, minValue), maxValue);

    if (clamped !== parsed || opts.force) {
        $input.val(clamped);
    }
    ...
    numberofsignatures = clamped;
    if (typeof PerpareDocumentContext === 'object' && PerpareDocumentContext !== null) {
        PerpareDocumentContext.signaturesRequiredCount = clamped;
    }
...
function validateNoOfSignatures(showErrors) {
    ...
    if (raw === '') {
        if (showErrors !== false && $error.length) {
            $error.text('Please enter a required signature count.');
    ...
    const state = updateRequiredSignatureState();
...
function bindRequiredSignatureInputHandlers() {
    ...
    $input.on('input', function () {
        updateRequiredSignatureState();
    });

    $input.on('change blur', function () {
        validateNoOfSignatures(true);
    });
}
...
$('#addSignatoriesCheckbox').off('change.multiSignCheckbox').on('change.multiSignCheckbox', function () {
    ...
    // Update workflow mode based on new recipient state
    updateWorkflowMode();
});
...
$('#AllSignatoriesRequired').change(function () {
    ...
    if (Array.isArray(window.recipients)) {
        window.recipients.forEach(function (_rec, idx) {
...
function buildRecipientCard(recipient, index) {
    ...
    const emailLabel = emailValue || 'No email specified';
        ...
        <div class="recipient-card recp" ${cardAttributes} id="recipient_card_${index}" draggable="true">
            ...
            <div class="recipient-header">
                ...
                <div class="recipient-controls">
                    <button type="button" class="move-up" title="Move up" aria-label="Move recipient up">
                        <i class="fa fa-arrow-up"></i>
                    </button>
                    <button type="button" class="move-down" title="Move down" aria-label="Move recipient down">
                        <i class="fa fa-arrow-down"></i>
                    </button>
                    <div class="${settingsClassName}" id="${settingsContainerId}">
                        <button type="button" class="btn btn-outline-primary settings-form-elements" id="${settingsButtonId}" data-settings-index="${index}">
                            <i class="fa fa-cog"></i>
                        </button>
                    </div>
                    <button type="button" class="remove-recipient" title="Remove recipient" aria-label="Remove recipient">
                        <i class="fa-regular fa-trash-can"></i>
                    </button>
                </div>
            </div>
              <label style="margin:0.25rem 0">Email</label>
            <div class="recipient-fields">
              
                <div class="recipient-email-wrap">
                    <input class="form-input recipient-email email" type="text" id="${emailInputId}" name="RecpientList[${index}].Email" value="${escapeHtmlAttr(emailValue)}" autocomplete="off" placeholder="E-Mail" data-val="true" data-val-regex="Please enter valid email" data-val-required="The Email field is required.">
...
function updateRecipientCountLabel() {
    const count = Array.isArray(window.recipients) ? window.recipients.length : 0;
    // Update badge with just the count number (more compact)
    $(recipientCardConfig.countLabel).text(count);
    $('#recipientCount').text(count);

    // Update workflow options visibility based on recipient count
    updateWorkflowMode();
}
...
function addRecipientCard(options) {
    ...
    // Update workflow mode indicators
    if (typeof updateWorkflowMode === 'function') {
        updateWorkflowMode();
    }
    ...
    if ($list.length) {
        ...
        $list.on('click', '.status-option', function (event) {
            ...
            if (!isNaN(idx) && Array.isArray(window.recipients) && window.recipients[idx]) {
                ...
                // Update min signatures display
                if (typeof updateMinSignaturesDisplay === 'function') {
                    updateMinSignaturesDisplay();
                }
            }
        });
        $list.on('input', '.recipient-email', function () {
            ...
            if (idx !== null && Array.isArray(window.recipients) && window.recipients[idx]) {
                ...
                $(this).closest('.recipient-card').find('.recipient-email-label').text(value || 'No email specified');
            }
        });
        $list.on('change', '.recipient-toggle input', function () {
            ...
            if (field) {
                ...
                if (field === 'mandatory' && typeof updateRequiredSignatureState === 'function') {
                    updateRequiredSignatureState();
                }
            }
        });
...
$("#Continue").off('click.documentFlow').on("click.documentFlow", async function (e) {
    ...
    if (modalobjlist.length > 0) {
        ...
        let integer_numberofsignatures = +numberofsignatures;
        if ((integer_numberofsignatures < mandatorycount) && (allSignaturesRequired == false)) {

            document.getElementById("navigationNetworkOverlay").style.display = "none";
            toastr.error(`Number of required signs is less than mandatory signatories`);
            e.preventDefault();
            return;
        }
        ...
        if ((integer_numberofsignatures < 0) && (allSignaturesRequired == false)) {

            document.getElementById("navigationNetworkOverlay").style.display = "none";
            toastr.error(`Please enter number of required signs greater than 0`);
            e.preventDefault();
            return;
        }
    } else {
