...
function updateSigningRequirementsCardVisibility() {
    const $card = $('#signatureConfigSection');
...
const RECIPIENT_DRAG_HANDLE_SELECTOR = '.recipient-drag-handle';

function setRecipientDragArmed($card, armed) {
    if (!$card || !$card.length) {
        return;
    }
    $card.attr('data-drag-armed', armed ? 'true' : 'false');
}
...
function updateDragDropState() {
    ...
    $cards.each(function () {
        ...
        setRecipientDragArmed($(this), false);
    });
}
...
function initializeDragAndDrop() {
    ...
    // Arm dragging only when interaction starts from drag handle
    $list.off('pointerdown.recipientDragArm').on('pointerdown.recipientDragArm', RECIPIENT_DRAG_HANDLE_SELECTOR, function () {
        if (getSigningModeType() !== 'sequential') {
            return;
        }
        const $card = $(this).closest('.recipient-card');
        setRecipientDragArmed($card, true);
    });

    // Any pointer interaction outside handle disarms dragging for that card
    $list.off('pointerdown.recipientDragDisarm').on('pointerdown.recipientDragDisarm', '.recipient-card', function (e) {
        if ($(e.target).closest(RECIPIENT_DRAG_HANDLE_SELECTOR).length > 0) {
            return;
        }
        setRecipientDragArmed($(this), false);
    });

    // If handle click ends without drag, disarm to avoid accidental future drags
    $list.off('pointerup.recipientDragArm pointercancel.recipientDragArm')
        .on('pointerup.recipientDragArm pointercancel.recipientDragArm', RECIPIENT_DRAG_HANDLE_SELECTOR, function () {
            const $card = $(this).closest('.recipient-card');
            setTimeout(function () {
                if (!$card.hasClass('dragging')) {
                    setRecipientDragArmed($card, false);
                }
            }, 0);
        });

    $(document).off('pointerup.recipientDragArmGlobal pointercancel.recipientDragArmGlobal')
        .on('pointerup.recipientDragArmGlobal pointercancel.recipientDragArmGlobal', function () {
            if ($('.recipient-card.dragging').length === 0) {
    ...
    $list.off('dragstart.recipientDrag').on('dragstart.recipientDrag', '.recipient-card', function (e) {
        ...
        const isDragArmed = $(this).attr('data-drag-armed') === 'true';
        if (!isDragArmed) {
            e.preventDefault();
            return false;
        }
...
function applyAccountTypeVisibility() {
    ...
    updateSigningRequirementsCardVisibility();
}
...
async function SaveQuickSignQrcode(qrcodedata, visibleAndInvisibleList) {
    ...
    if (hasFalseValue) {
        ...
        document.getElementById('overlay9-default').style.display = 'block';
        document.getElementById('overlay9-custom').style.display = 'none';
...
async function SaveQuickSign(visibleAndInvisibleList) {
    ...
    if (Object.keys(visibleAndInvisibleList).length !== 0) {
        ...
        document.getElementById('overlay9-default').style.display = 'none';
        document.getElementById('overlay9-custom').style.display = 'block';
        document.getElementById('overlay9').style.display = 'flex';
        okElement = document.getElementById('okButton');
        cancelElement = document.getElementById('cancelButton')
    }
                    ...
                    if (Object.keys(userDataResponses[i]).length !== 0) {
                        ...
                        if (i === orderId) {
                            ...
                            if (emailArray.includes(emailInputValue.trim()) || rowEmail_value === emailInputValue.trim()) {
                                toastr.error(`Email of the subscriber already exists.`);
                ...
                if (response.userProfile == null || $.isEmptyObject(response.userProfile)) {
                    ...
                    if (modalobjdata_val.selectedorg.orgName == "Self") {
                        if (userdetails.ugpassEmail != email) {
                            toastr.error(`The signatory is not self signatory ${email}`);
                            return;
                        }
                    }
                    if (modalobjdata_val.selectedorg.orgName !== "Self" || modalobjdata_val.selectedorg.orgUid.trim() != "") {
                        if (isEsealRequired) {
                            if (!modalobjdata_val.selectedorg.eseal_employee_list.includes(email)) {
                                toastr.error(`Signatory does not have e-seal permission ${email}`);
                                return;
                            }
...
function validateAlternateEmailobj(settingsIndex, selectedorg) {
    ...
    $('.email-input1').each(function () {
        ...
        if (!isEseal && !selectedorg.employee_list.includes(emaildata)) {
            ...
            toastr.error(`The assigned signatory does not belong to the selected organization`);
            alternativeSignatories_obj.bool = false;
            return false; // Exit early
        }
...
function validateAlternateEmail(settingsIndex, selectedorg) {
    ...
    $('.email-input1').each(function () {
        ...
        if (emailValidFlag) {
            ...
            toastr.error(`Signatory cannot assign himself as alternative signatory`);
            isAlternateEmailsValid = false;
            return false; // Exit early
        ...
        for (let i = 0; i < alternateSignatoriesList.length; i++) {
            ...
            for (let j = 0; j < signatories.length; j++) {
                ...
                if (signatories[j].email === emaildata) {
                    ...
                    toastr.error(`This user is already an alternative signatory`);
                    isAlternateEmailsValid = false;
                    return false; // Exit early
                }
            }
        ...
        if (userDataResponses[settingsIndex].rowEmail === emaildata) {
            ...
            toastr.error(`You cannot assign yourself as alternative signatory`);
            isAlternateEmailsValid = false;
            return false; // Exit early
        }
        ...
        if (isError) {
            if (isEmailMatched) {
                ...
                toastr.error(`Assigned signatory does not belong to the selected organization`);
                ...
                toastr.error(`Assigned signatory does not belongs to selected organization`);
            }
            isAlternateEmailsValid = false;
            return false; // Exit early
        } else {
...
function isRecipientAccountSelectionValid(rec) {
    if (!rec || typeof rec !== 'object') {
        return false;
    }

    const accountUid = (rec.accountUid || '').toString().trim();
    if (accountUid.length > 0) {
        return true;
    }

    const accountName = (rec.accountName || '').toString().trim().toLowerCase();
    return accountName === 'self';
}
...
async function gatherRecipients(options) {
    ...
    try {
        ...
        for (let i = 0; i < normalized.length; i++) {
            ...
            if (!isRecipientAccountSelectionValid(rec) && $('#addSignatoriesCheckbox').prop('checked')) {
                response.ok = false;
...
$("#Continue").off('click.documentFlow').on("click.documentFlow", async function (e) {
    ...
    if (loginesealsignatoryrole === 'False' && esealcheckboxval && !isChecked) {
        ...
        swal({
            type: 'info',
            title: 'Info',
            text: `You don't have eseal permission for logged in organization`
    ...
    if (modalobjlist.length > 0) {
        ...
        if (suidslistarray.length > uniquesuidslist.length) {
            document.getElementById("navigationNetworkOverlay").style.display = "none";
            toastr.error("Email of the subsciber already exists.");
            e.preventDefault();
            return;
        }
        if (PerpareDocumentContext.MultiSign === true) {
...
function updateStepper(currentStep, state, msg, esealcorddata, signcorddata, data, tempidfordetails, resp, blob) {
    ...
    if (state == "success") {
        const messages = ["Check for " + currentStaging + " mobile app notification and approve signing request...",
            "Document signing in progress. Please wait.",
                ...
                .then((response) => {
                    if (response.status == "Success") {
                        ...
                        swal({
                            ...
                            // ✅ SweetAlert v1 custom icon
                            imageUrl: '/img/Tickimage.png',
                            imageWidth: 80,
                            imageHeight: 80,
    ...
    if (file) {
        ...
        reader.onload = async function (event) {
            ...
            if (file.type == "application/pdf") {
                ...
                document.getElementById("fileSizeDisplay").textContent = sizeMB;
...
function applyZoom() {
    const pdfContainer = (typeof window !== 'undefined' && window.pdfContainer) ||
        ...
        if (checkbox.checked === true) {
            if (selectedorderobj.selectedorgList[0].has_eseal_permission === false) {
                if (selectedorderobj.selectedorgList[0].orgUid.trim() == "" && selectedorderobj.selectedorgList[0].orgName == "Self") {
                    ...
                    toastr.error(`You don't have eseal permission for self account`);
...
// ============================================================
// UI INITIALIZATION & EVENT SETUP (Extracted from .cshtml)
// ============================================================
// These functions were previously embedded in CreateDocumentsNew.cshtml
// Now extracted for cleaner separation of concerns
// 
// Razor variables are passed from .cshtml as window globals:
// - window.loginuserAccount, window.loginEmail, window.currentStaging, etc.
// - window.workerpath, window.globalFile, etc.

/**
 * Initializes UI on page load
 * Shows upload screen by default, hides prepare screen
 */
window.initializeUI = function () {
    // Show upload screen by default
    $('#uploadScreen').removeClass('hidden');
    $('#prepareScreen').removeClass('hidden').addClass('hidden');
};

/**
 * Sets up all event listeners for file upload and workflow controls
 */
window.setupEventListeners = function () {
    // Dropzone click to browse (keeps existing hidden input flow)
    $('#dropzone').on('click', function (e) {
        e.preventDefault();
        // Try to open the existing hidden input first
        const $file = $('#File');
        if ($file.length) {
            try {
                $file.trigger('click');
            } catch (err) {
                // fallback to creating a temporary input below
                window.openTemporaryFileInput();
            }
        } else {
            window.openTemporaryFileInput();
        }
    ...
    // Robust Browse Files button: create a temporary input and open it.
    // Stop propagation so the parent dropzone click handler doesn't also fire (causes double dialog).
    ...
    // File input change
    $('#File').on('change', window.handleFileSelect);

    // Drag and drop
    window.setupDragAndDrop();

    // Cloud providers
    $('#googleDirve, #googleDrive').on('click', function () {
        console.log('Google Drive clicked');
        // Add your Google Drive integration logic
    });

    $('#oneDrive').on('click', function () {
        console.log('OneDrive clicked');
        // Add your OneDrive integration logic
    });

    // Cancel button
    $('#cancelBtn').on('click', function () {
        window.resetToUploadScreen();
    });

    // Continue button - handled by legacy CreateDocuments.js at line 3896
    // The Continue button click handler is in CreateDocuments.js which:
    // 1. Validates file upload, document name, days to complete
    // 2. Validates all recipient emails and organization selections
    // 3. Shows #navigationNetworkOverlay during processing
    // 4. Transitions from prepare screen to viewer (#viwer)
    // 5. Loads PDF and initializes annotation layer
    // Legacy handler will be bound when CreateDocuments.js loads

    // Days to complete change
    $('#DaysToComplete').on('change', window.updateExpiryDate);

    // Recipient add/remove/reorder handled by CreateDocuments.js (recipient card module)

    window.bridgeLegacyWorkflowToggles();

    // Initialize toastr CDN assets and account dropdown UI helpers
    window.ensureToastrAssets();
    window.initializeAccountDropdownHelpers();
};

/**
 * Opens an external file chooser dialog
 * Creates a temporary file input, attaches it to DOM, and triggers click
 */
window.openTemporaryFileInput = function () {
    const temp = $('<input type="file" accept=".docx,.pdf,.xlsx" style="display:none">');
    $('body').append(temp);
    temp.on('change', function (evt) {
        if (this.files && this.files[0]) {
            window.handleFileUpload(this.files[0]);
        }
...
/**
 * Bridges legacy workflow toggle behavior with modern controls
 * Syncs between old (#AllSignatoriesRequired, #DiasbleOrder) and new controls
 */
window.bridgeLegacyWorkflowToggles = function () {
    const $legacyAllRequired = $('#AllSignatoriesRequired');
    const $allRequiredToggle = $('#allSignersRequiredToggle');

    if ($allRequiredToggle.length) {
        if ($legacyAllRequired.length) {
            $allRequiredToggle.prop('checked', $legacyAllRequired.prop('checked'));
        }

        $allRequiredToggle.on('change', function () {
            const isChecked = $(this).is(':checked');
            if ($legacyAllRequired.length) {
                $legacyAllRequired.prop('checked', isChecked).trigger('change');
            }
        });

        if ($legacyAllRequired.length) {
            $legacyAllRequired.on('change.bridgeAllSigners', function () {
                const state = $(this).is(':checked');
                if ($allRequiredToggle.prop('checked') !== state) {
                    $allRequiredToggle.prop('checked', state);
                }
            });
        }
    }

    const $sequentialToggle = $('#sequentialSigning');
    const $legacyDisableOrder = $('#DiasbleOrder');

    if ($sequentialToggle.length) {
        if ($legacyDisableOrder.length) {
            $sequentialToggle.prop('checked', !$legacyDisableOrder.prop('checked'));
        }

        $sequentialToggle.on('change', function () {
            const disableOrder = !$(this).is(':checked');
            if ($legacyDisableOrder.length) {
                $legacyDisableOrder.prop('checked', disableOrder);
            }
            if (window.PerpareDocumentContext && typeof window.PerpareDocumentContext === 'object') {
                window.PerpareDocumentContext.disableOrder = disableOrder;
            }
        });
    }
};
...
/**
 * Sets up drag and drop event handlers for the file upload zone
 */
 ...
 * Delegates to legacy fileSelect function for backward compatibility
 */
window.handleFileUpload = function (file) {
    try {
        const fakeEvent = { target: { files: [file], value: '' } };
        if (typeof fileSelect === 'function') {
            fileSelect(fakeEvent).catch(err => console.error('fileSelect error:', err));
        } else {
            console.warn('Legacy fileSelect function not available');
        }
    } catch (err) {
        console.error('Error delegating to legacy fileSelect:', err);
    }
};

/**
 * Updates the file info display in the prepare screen
 */
window.updateFileInfo = function (file) {
    const fileName = file.name;
    const fileSize = (file.size / 1024 / 1024).toFixed(2) + ' MB';
    const fileType = window.getFileExtension(fileName).toUpperCase();

    $('#fileNameDisplay').text(fileName);
    $('#fileSizeDisplay').text(fileSize);
    $('#fileTypeDisplay').text(fileType);
    $('#DocumentName').val(fileName.replace(/\.[^/.]+$/, ''));
    ...
    $('.file-icon-large i').removeClass().addClass('fa ' + (iconMap[fileType] || 'fa-file'));
};
...
/**
 * Shows the prepare screen and hides the upload screen
 */
window.showPrepareScreen = function () {
    $('#uploadScreen').addClass('hidden');
    $('#prepareScreen').removeClass('hidden').addClass('active');
    window.updateExpiryDate();
};

/**
 * Resets to upload screen, clearing file selection
 */
window.resetToUploadScreen = function () {
    $('#prepareScreen').removeClass('active').addClass('hidden');
    $('#uploadScreen').removeClass('hidden');
    $('#File').val('');
    window.globalFile = null;
    if (typeof isFileUploaded !== 'undefined') {
        isFileUploaded = false;
    }
};

/**
 * Updates the expiry date display based on days to complete
 */
window.updateExpiryDate = function () {
    const days = parseInt($('#DaysToComplete').val()) || 7;
    const today = new Date();
    const expiryDate = new Date(today.getTime() + (days * 24 * 60 * 60 * 1000));
    const options = { year: 'numeric', month: 'long', day: 'numeric' };
    $('#expiryDate').text(expiryDate.toLocaleDateString('en-US', options));
};

/**
 * Google API callback for loading picker and auth APIs
 */
...
/**
 * Ensures toastr library (CSS + JS) is loaded and available
 * Injects toastr assets from CDN if not already present
 */
window.ensureToastrAssets = function () {
    var CDN_CSS = '/local-cdns/Shared/toastr/latest/toastr.min.css';
    var CDN_JS = '/local-cdns/Shared/toastr/latest/toastr.min.js';
    ...
    function injectCss() {
        if (hasAsset('link[href*="toastr.min.css"]')) {
            return;
        }
        var link = document.createElement('link');
        link.rel = 'stylesheet';
        link.href = CDN_CSS;
        link.setAttribute('data-toast-cdn', 'true');
    ...
    function injectJs() {
        ...
        var existing = document.querySelector('script[data-toast-cdn="true"]');
        if (existing) {
            if (!existing.__toastReadyHooked) {
                existing.__toastReadyHooked = true;
        ...
        script.src = CDN_JS;
        script.async = false;
        script.setAttribute('data-toast-cdn', 'true');
...
/**
 * Initializes account dropdown UI helpers
 * Manages focus visual and improves arrow click target for recipient account dropdown
 * Non-invasive: does not change selection logic
 */
window.initializeAccountDropdownHelpers = function () {
    $(document).off('focus.accountDropdown blur.accountDropdown click.accountDropdown').on('focus.accountDropdown', '.recipient-account-dropdown .account-select', function () {
        $(this).closest('.account-wrapper').addClass('open');
    }).on('blur.accountDropdown', '.recipient-account-dropdown .account-select', function () {
        const $wrap = $(this).closest('.account-wrapper');
        setTimeout(function () { $wrap.removeClass('open'); }, 160);
    }).on('click.accountDropdown', '.recipient-account-dropdown .account-wrapper', function (e) {
        const $sel = $(this).find('.account-select');
        if ($sel.length) {
            // Focus and trigger native opening behavior where supported
            $sel.focus();
            try { $sel[0].dispatchEvent(new MouseEvent('mousedown')); } catch (err) { }
        }
    }).on('click.accountDropdown', function (e) {
        if (!$(e.target).closest('.recipient-account-dropdown').length) {
            $('.recipient-account-dropdown .account-wrapper.open').removeClass('open');
        }
    });
};
