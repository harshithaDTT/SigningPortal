...
$(document).ready(function () {
    ...
    if ($.fn.DataTable.isDataTable('#ReferredTb')) {
        $('#ReferredTb').DataTable().destroy(); // Destroy the existing instance
        $('#ReferredTb').empty();
    }

    var table = $('#ReferredTb').DataTable({
        searching: false,
        serverSide: true,
        paging: false,
        ordering: false,
            ...
            function fetchDocuments() {
                ...
                currentSentAjaxRequest = $.ajax({
                    ...
                    success: function (res) {
                        ...
                        //      if ($('#referredCardsContainer').length) {
                        //             $('#referredCardsContainer').empty();
                        //      }
                        // }
                        ...
                        if (res.data.length === 0 && currentPage === 0) {
                            ...
                            if ((currentPage + 1) * pageSize >= totalRecords) {
                                ...
                                if (totalRecords > pageSize) {
                                    $('#allDocsLoadedMsg').show();
                                    document.getElementById("allDocsLoadedMsg").innerHTML = `All documents loaded (${totalRecords})`
                                }
    ...
    $('#searchButton').on('click', function () {
        ...
        $('#noDataMessageref').hide();
        $('#showMoreBtnref').hide();
        $('#ReferredTb').DataTable().ajax.reload();
    });
...
//                                                          <div style="display:${anyVisible ? "block" : "none"};" class="checkboxContainer toggle-row custom-checkbox-wrapper ${makeDisabled ? 'disabled' : ''}">
...
// function renderActionButtonHtml(item, suid) {
...
//     if (!rec) return `<button type='button' class='${baseBtn}' style='${style}' onclick='redirectToDocumentDetails("${id}")'>
...
//         switch (rec.status) {
//             case "Need to sign":
//                 return `<button type='button' class='${baseBtn}' style='${style}' onclick='signActionConfigByDocIdOnclick("${id}", false)'>
//                             <i class='fa fa-pencil'></i>&nbsp;Sign</button>`;
//             case "Signing":
//             case "Signing in progress":
//             case "Signed":
//                 return `<button type='button' class='${baseBtn}' style='${style}' onclick='redirectToDocumentDetails("${id}")'>
//                             <i class='fa-regular fa-eye'></i>&nbsp;View</button>`;
//             case "Pin Failed":
//             case "Failed":
//                 return `<button type='button' class='${baseBtn}' style='${style}' onclick='signActionConfigByDocIdOnclick("${id}")'>
