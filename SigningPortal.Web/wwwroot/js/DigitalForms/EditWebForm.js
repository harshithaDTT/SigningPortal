...
$(document).ready(function () {
    ...
    $("#rolesNextBtn").on("click", Roles);

    $("#addApproverRoleBtn").on("click", function () {
        addNewRole();
    });

    $("#prevtoDocConfigBtn").on("click", function () {
        DocConfig();
    });

    $("#prevRoleBtn").on("click", function () {
        Roles();
    });

    $("#nextRoleConfigBtn").on("click", function () {
        RolesConfiguraton();
    });
    ...
    $("#imageid").on("click", function () {
        handleAddimage('image field', true);
    });

    $("#rfullname").on("click", function () { handleAddRecommended('Full Name', 'Full Name', true); });
    $("#rgender").on("click", function () { handleAddRecommended('Gender', 'Gender', true); });
    $("#remail").on("click", function () { handleAddRecommended('Email', 'email', true); });
    $("#rphone").on("click", function () { handleAddRecommended('Phone Number', 'Phone Number', true); });
    $("#rdate").on("click", function () { handleAddRecommended('Date of Birth', 'Date of Birth', true); });
    $("#rfname").on("click", function () { handleAddRecommended('First Name', 'firstname', true); });
    $("#rLastName").on("click", function () { handleAddRecommended('Last Name', 'lastname', true); });
    $("#rNation").on("click", function () { handleAddRecommended('Nation', 'Nation', true); });
    $("#rSelfie").on("click", function () { handleAddimage('Selfie', true); });

    $("#signatureid1").on("click", function () { handleSignature(true); });
    $("#esealid1").on("click", function () { handleEseal(true); });


    $('#daysToComplete').on('input', function (event) {
        validateInput(event);
    });

    $('#input_1').on('input', function (event) {
        validateRole(event);
    });
...
function RolesConfiguraton() {
    ...
    if (roleList.length == 1) {
        ...
        if ((roleList[1].name).replace(/\s+/g, '') === '') {
...
async function UpdateForm() {
    ...
    if (missingroles.length > 0) {
        ...
        docConfig.documentName = document.getElementById("tempName").value;
        docConfig.htmlSchema = JSON.stringify(annotations);
        docConfig.sequentialSigning = $("#sequentialSign").prop('checked');
        docConfig.numberOfSignatures = document.getElementById("NoOfSign").value;
