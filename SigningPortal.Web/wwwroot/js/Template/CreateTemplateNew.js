...
$(document).ready(function () {
    ...
    // Load signatory profile images and names
    loadAllSignatoryProfiles();

    $("#browseFilesButton").on("click", open_file);
    ...
    $("#templatePreviewBtn").on("click", function () {
        ImagePreview();
    });

    $("#esealTemplatePreviewBtn").on("click", function () {
        ImagePreviewEseal();
    });
...
$('.select-checkbox').change(function () {
    // Single-select behavior: only one signatory can be selected at a time
    var $this = $(this);
    var $card = $this.closest('.signer-card, .signatory-tile');

    // If this is a radio input (new card design), handle selection
    if ($this.attr('type') === 'radio') {
        // Clear all previous selections visually
        $('.signer-card, .signatory-tile').removeClass('selected');
        ...
        // Set as the single selected email
        ...
        if (continueBtn && isFileUploaded) {
            continueBtn.disabled = false;
            continueBtn.classList.remove('disabled-blur');
        }
    } else {
        // Legacy checkbox behavior (backward compatibility)
        // If checkbox is checked
        if ($this.prop('checked')) {
            // Uncheck all other checkboxes (enforce single selection)
            $('.select-checkbox').not(this).prop('checked', false);
            $('.select-checkbox').not(this).closest('.signer-card, .signatory-tile, .bulk-checkbox').removeClass('selected');
...
// Also handle click on signer cards for better UX
$(document).on('click', '.signer-card, .signatory-tile', function (e) {
    // Only handle if not clicking directly on the radio input
...
function Settings() {
    // With single-select, there should always be exactly 0 or 1 selection
...
function closeSettingsModal() {
    ...
    document.getElementById('templatePreviewBtn').style.display = 'none';
    document.getElementById('esealTemplatePreviewBtn').style.display = 'none';
...
document.getElementById('templateSelect').addEventListener('change', function () {
    ...
    if (selectedValue !== "0") {
        document.getElementById('templatePreviewBtn').style.display = 'block';
    } else {
        document.getElementById('templatePreviewBtn').style.display = 'none';
    }
});
...
document.getElementById('EsealtemplateSelect').addEventListener('change', function () {
    ...
    var selectedValue = this.value;
    if (selectedValue !== "0") {
        document.getElementById('esealTemplatePreviewBtn').style.display = 'flex';
    } else {
        document.getElementById('esealTemplatePreviewBtn').style.display = 'none';
    }
});
...
function InnerDiv() {
    var model = document.getElementById("templatePreviewBtn");
    if (model.style.display === "none") {
...
function InnerEsealDiv() {
    var model = document.getElementById("esealTemplatePreviewBtn");
    if (model.style.display === "none") {
        model.style.display = "flex";
    } else {
        model.style.display = "none";
...
function PrepareDocument() {
    ...
    PerpareDocumentContext.receipientEmailsList.map((item, index) => {
        ...
        $("#emailList").append(` <span style="display: inline-flex;align-items: center;font-size: 15px;"><img src="${thumbnail_Img}" class="selected2" alt="Thumbnail"><span style="font-size:0.9rem;text-overflow: ellipsis;overflow: hidden;max-width: 200px;display: inline-block;border: none;background: none;box-shadow: none;padding: 0;" title="${item.email}"> ${item.email}</span></span>`);
...
function showUploadedState() {
    setTemplateFlowState(true);
}

function resetUploadedState(options = { clearFileInput: true, clearRecipients: true }) {
    setTemplateFlowState(false);
    $('#DocumentName').val('');
...
function setTemplateFlowState(isFileReady) {
    var $uploadScreen = $('#templateUploadScreen');
    var $prepareScreen = $('#templatePrepareScreen');

    if (!$uploadScreen.length || !$prepareScreen.length) {
        return;
    }

    if (isFileReady) {
        $uploadScreen.addClass('classshide').attr('aria-hidden', 'true');
        $prepareScreen.removeClass('classshide').attr('aria-hidden', 'false');
        $('#Continue').removeAttr('disabled').removeClass('disabled-blur');
    } else {
        $prepareScreen.addClass('classshide').attr('aria-hidden', 'true');
        $uploadScreen.removeClass('classshide').attr('aria-hidden', 'false');
        $('#Continue').attr('disabled', 'disabled').addClass('disabled-blur');
        $('#viwer').addClass('classshide');
    }
}

function initializeTemplateFlow() {
    var existingFileBase64 = ($('#filebase64').val() || '').trim();
    var hasExistingFile = isFileUploaded || existingFileBase64.length > 0;
    setTemplateFlowState(hasExistingFile);
}
...
$(document).on("click", "#changeDocumentBtn", function () {
    resetUploadedState();
});
...
// Defer initial binding to document ready to ensure DOM is parsed
// Actual activation happens when PDF viewer opens via initDragAndDrop()
