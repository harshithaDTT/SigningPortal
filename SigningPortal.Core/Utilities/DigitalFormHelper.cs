...
{
	public class DigitalFormHelper : IDigitalFormHelper
	{
		private readonly IConfiguration _configuration;

		public DigitalFormHelper(IConfiguration configuration)
		{
			_configuration = configuration;
		}

		public string GenerateHtmlString(FillFormData data)
		{
			var rolesData = JsonConvert.SerializeObject(data.Roles);
			string url;
			var environment = Environment.GetEnvironmentVariable("ASPNETCORE_ENVIRONMENT");
			var isDevelopment = environment == Environments.Development;
			if (isDevelopment)
			{
				url = _configuration.GetValue<string>("CallBackUrl") + "/api/forms/saveandsign";
			}
			else
			{
				url = _configuration.GetValue<string>("Config:Signing_portal:BACKEND_URL") + "/api/forms/saveandsign";
			}
    ...
    <meta name=""viewport"" content=""width=device-width, initial-scale=1.0"">
                    ...
                    .image-container {{background - color: #d8d7d78a;
        color: #636161;
    ...
    .image-container:hover .remove-image-btn.uploaded {{display: block;
    }}
...
#pdf-container::-webkit-scrollbar-thumb {{background: #888; 
  border-radius: 10px; 
}}
...
<div class=""loader-container"" id=""loader-container"">
        <div class=""loader"" id=""circle-loader""></div>
    </div>
                    <div class=""row"" style=""height:100%"">
                        <div id=""container"" class=""col-md-12"" style=""height:100%"">
                                    ...
                                    <div id=""leftpanel"" class=""row"" style=""display: flex"">
                                        <div class=""col-md-12 "" style=""border: 1px solid #ccc; flex: 2;padding:0;"">

                                            

                                            <div id=""pdfouter"" style="" height:100%;"">
                                                <div id=""pdf-container"" style=""max-height:100%; overflow-y: auto;""></div>
                                            </div>
                                        </div>
                                      </div>


                                        <div id=""rightpanel"" class=""row"" style=""display:flex;align-items:flex-start;justify-content:center;gap:13%"">
                                    
                                                     <button id=""fill"" style=""box-shadow:0 0px 6px 0 rgb(0 0 0 / 20%)"">
                                                       Auto Fill
                                                    </button>
                                                    <button id=""save"" style=""box-shadow:0 0px 6px 0 rgb(0 0 0 / 20%)"">
                                                       Save & Sign
                                                    </button>
                        ...
                        <input type=""hidden"" id=""filebase64"" name=""filebase64"" value={data.pdfblob} />
                    ...
                    <script>
                        ...
                        $(document).ready(function () {{
                            ...
                            const components = {data.htmlschma};
                ...
                return element;
                        ...
                        async function saveAnnotations() {{
                            ...
                            if(canSave){{
                                ...
                                var autodata = {data.autofilldata};
                                if (autodata == """") {{
                                     return swal({{
                                         title: ""Info"",
                                ...
                                if (issignpresent && isesealpresent){{
                                    pinType = ""Both"";
                                    }}else if(issignpresent == true && isesealpresent == false){{
                                    pinType = ""Signing PIN""
                                    }}else{{
                                        pinType = ""Authentication PIN""
                                        }}
                            
                                if (window.AndroidInterface && window.AndroidInterface.sendData) {{
       
                                
                                    AndroidInterface.sendData(pdfBase64, json, pinType);
                                }} else if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.sendData) {{
                                    ...
                                    console.error('No native interface available to send data.');
                                }}
    ...
    if (document.getElementById('birthdate')) {{
		...
		public string GenerateSimHtmlString(FillFormData data)
		{
			var rolesData = JsonConvert.SerializeObject(data.Roles);
			string url;
			var environment = Environment.GetEnvironmentVariable("ASPNETCORE_ENVIRONMENT");
...
<div class=""loader-container"" id=""loader-container"">
        <div class=""loader"" id=""circle-loader""></div>
                    ...
                    <script>
                        ...
                        function renderPDF(pdf, annotations = []) {{
                            ...
                            if(numPages === 1){{
                               pdfContainer.classList.add('singlepage'); 
                            }}
                            for (let pageNum = 1; pageNum <= numPages; pageNum++) {{
                                pdf.getPage(pageNum).then(page => {{
                                    ...
                                    if(numPages === 1){{
                                    pdfContainer.style.height = canvas.height;
                                    }}
                                    ...
                                    page.render(renderContext).promise.then(() => {{
                                        ...
                                        if (pageAnnotations.length > 0) {{
                                            pageAnnotations.forEach(annotation => {{
                                                if (annotation.type !== 'Wsave' && annotation.type !== 'Wfill' && annotation.type !== 'Signature') {{
                                                    const element = createDraggableElement(annotation);
                                                    annotationLayer.appendChild(element);
                                                }}
                                            }});
                ...
                return element;
                        ...
                        async function saveAnnotations() {{
                            ...
                            if(canSave){{
                                ...
                                annotations[""Image""] = autodata.portrait;
                                canvases.forEach(canvas => {{
                                ...
                                document.querySelectorAll('.draggable .input-field').forEach(input => {{
                                    if (input.type === 'text') {{
                                        ...
                                        annotations[(input.id).split('_')[0]] = input.value;
        ...
        if (document.getElementById(`Phone Number_${{userrole}}`)) {{
            document.querySelectorAll(`[id=""Phone Number_${{userrole}}""]`).forEach((input) => {{
                ...
                if (input.type === ""text"" && input.value.trim() === """") {{
                    fieldsFilled += 1;
                    input.value = autodata.phoneNumber;
                    input.classList.add('has-value');
                }}
            }});
            ...
            if (document.getElementById(`passport number_${{userrole}}`)) {{
        document.querySelectorAll(`[id=""passport number_${{userrole}}""]`).forEach((input) => {{
            totalAutofillFields += 1;
            if (input.type === ""text"" && input.value.trim() === """") {{
                fieldsFilled += 1;
                input.value = autodata.documentNumber;
                input.classList.add('has-value');
            }}
        }});
    }}
        ...
        if (document.getElementById(`Full Name_${{userrole}}`)) {{
            document.querySelectorAll(`[id=""Full Name_${{userrole}}""]`).forEach((input) => {{
                ...
                if (input.type === ""text"" && input.value.trim() === """") {{
                    fieldsFilled += 1;
                    input.value = autodata.givenName;
                    input.classList.add('has-value');
                }}
            }});
        }}
        ...
        document.querySelectorAll(`[id=""Nation_${{userrole}}""]`).forEach((input) => {{
            ...
            if (input.type === ""text"" && input.value.trim() === """") {{
                fieldsFilled += 1;
                input.value = autodata.issuingCountry;
                input.classList.add('has-value');
            }}
        }});
    }}

if (document.getElementById(`New Phone Number_${{userrole}}`)) {{
        document.querySelectorAll(`[id=""New Phone Number_${{userrole}}""]`).forEach((input) => {{
            totalAutofillFields += 1;
            if (input.type === ""text"" && input.value.trim() === """") {{
                fieldsFilled += 1;
                input.value = autodata.newPhoneNumber;
                input.classList.add('has-value');
            }}
        }});
    }}

if (document.getElementById(`Sim Number_${{userrole}}`)) {{
        document.querySelectorAll(`[id=""Sim Number_${{userrole}}""]`).forEach((input) => {{
            totalAutofillFields += 1;
            if (input.type === ""text"" && input.value.trim() === """") {{
                fieldsFilled += 1;
                input.value = autodata.simId;
                input.classList.add('has-value');
            }}
        }});
    }}
    ...
    if (document.getElementById(`Date of Birth_${{userrole}}`)) {{
        document.querySelectorAll(`[id=""Date of Birth_${{userrole}}""]`).forEach((input) => {{
            ...
            if (input.type === ""text"" && input.value.trim() === """") {{
                fieldsFilled += 1;
                input.value = autodata.birthDate;
                input.classList.add('has-value');
            }} else if (input.type === ""date"" && input.value.trim() === """") {{
                fieldsFilled += 1;
                input.value = autodata.birthDate;
    ...
    if (document.getElementById('birthdate')) {{
        ...
        document.querySelectorAll(`[id=""Selfie_${{userrole}}""]`).forEach((input) => {{
            ...
            if (input.tagName.toLowerCase() === 'img' && input.src.trim() === """") {{
                fieldsFilled += 1;
                if (autodata.portrait.startsWith(""data:image"")) {{
                      input.setAttribute(""src"", autodata.portrait);
                    }} else {{
                      input.setAttribute(""src"", ""data:image/jpeg;base64,"" + autodata.portrait);
                    }}
                input.style.display = 'block';
                if (input.nextElementSibling) {{
                    input.nextElementSibling.style.display = 'none';
		...
		public string WebhtmlString(FillFormData data)
		{
			var rolesData = JsonConvert.SerializeObject(data.Roles);
			string url;
			var environment = Environment.GetEnvironmentVariable("ASPNETCORE_ENVIRONMENT");
...
background-color: #ACC43D;
    ...
    .cdx-inline-tool-button {{
        display: inline-flex;
        align-items: center;
    }}

        .cdx-inline-tool-button select {{
            margin-left: 5px;
        }}
    ...
    .button-content {{
        display: flex;
        align-items:center;
        justify-content:center; /* Distribute space between icon and text */
        font-size: 16px; /* Adjust font size */
        width: 100%; /* Ensure the button content spans the entire button */
    }}
...
<div class=""loader-container"" id=""loader-container"">
        <div class=""loader"" id=""circle-loader""></div>
                    ...
                    <div class=""row"" style=""height:100%"">
                        <div id=""container"" class=""col-md-12"" style=""height:100%;padding-left:0;padding-right:0;"">
                          ...
                          <div id=""pdfouter"" style="" height:100%;"">
                        <div class=""row"" id=""pdf-container"" style=""overflow-y:scroll; height:92%;width:100%;margin-left:auto;margin-right:auto;justify-content:center"">
                            </div>
                           </div>
                                        </div>
                                      </div>
                                        ...
                                        <div id=""rightpanel"" class=""row"" style=""display:flex;align-items:flex-start;justify-content:center;gap:13%"">
                                                     ...
                                                     <button id=""fill"" style=""box-shadow:0 0px 6px 0 rgb(0 0 0 / 20%)"" onclick=""autofill()"">
                                                       Auto Fill
                                                    </button>
                                                    <button id=""save"" style=""box-shadow:0 0px 6px 0 rgb(0 0 0 / 20%)"" onclick=""saveAnnotations()"">
                                                       Save & Sign
                                                    </button>
 ...
 class ParagraphBlock {{
   ...
   function customHeaderparser(block) {{
    ...
    document.addEventListener('DOMContentLoaded', () => {{
        const editorInstancesData = {data.htmlschma};
...
function arrayBufferToBase64(buffer) {{
  ...
  if (isnotpassed) {{
     ...
     var issignpresent = false;
     var pinType;
     const signele = document.getElementById('Signature'  + '_' + userrole)
     if (signele) {{
         overlaysignflag = true;
         issignpresent = true;
     }}
     const esealele = document.getElementById('Eseal'  + '_' + userrole)
     if (esealele) {{
         overlayesealflag = true;
         isesealpresent = true
     }}
  ...
  if (document.getElementById('Selfie')) {{
    ...
    loaderContainer.style.display = 'none'; 
		...
		public string ViewFilledForm(FillFormData data)
		{
			string viewString =
				$@"
...
#pdf-container{{width:100%
            ...
            #downloadButton {{font - size:0.8rem;
			   padding: 5px 10px;
                margin-top:1%;
...
<div class=""loader-container"" id=""loader-container"">
        <div class=""loader"" id=""circle-loader""></div>
                    ...
                    <div class=""row"" style=""height:100%"">
                        <div id=""container"" class=""col-md-12"" style=""height:100%"">
                                    ...
                                    <div id=""leftpanel"" class=""row"" style=""display: flex"">
                                       ...
                                       <div id=""rightpanel"" class=""row"" style=""display: flex; align-items: center; justify-content: center; gap: 13%;"">
                                            
                                            <button id=""downloadButton"" onclick=""downloadPDF()"" style=""box-shadow:0 0px 6px 0 rgb(0 0 0 / 20%)"">
                                                     Download
                                                    </button>
                                        </div>
                        </div>
                    ...
                    <script>
                        ...
                        function downloadPDF() {{
                            ...
                            var pdfBase64 = document.getElementById('filebase64').value;
                            var json = """";
                            var pinType = """";
		...
		public string RequestGenerateHtmlString(FillFormData data)
		{
			var rolesData = JsonConvert.SerializeObject(data.Roles);
			string url;
			var environment = Environment.GetEnvironmentVariable("ASPNETCORE_ENVIRONMENT");
...
<script>
    ...
    $(document).ready(function () {{
        ...
        var components;
        if (forweb1 != ""onlypdf"") {{
            components = {data.pdfschema}
        }}
        else {{
            components = {data.htmlschma}
        }}
    ...
    function createDraggableElement(annotation) {{
        if (annotation.role === userrole || (annotation.type === 'plain-text' && status1 === ""New"")) {{
            if (annotation.type === 'text') {{
                isannotationspresent = false;
                const element = document.createElement('div');
                element.classList.add('draggable');
    ...
    async function saveAnnotations() {{
        ...
        if (secondsign !== """") {{
            ...
            if (isannotationspresent) {{
                ...
                showLoader()
                const pdfData = await capturePDFWithAnnotations();
                pdfBytes = await createPDFWithImages(annotationspages, pdfData);
    ...
    if (document.getElementById('birthdate')) {{
		...
		public string RequestWebhtmlString(FillFormData data)
		{
			var rolesData = JsonConvert.SerializeObject(data.Roles);
			string url;
			var environment = Environment.GetEnvironmentVariable("ASPNETCORE_ENVIRONMENT");
...
<div class=""loader-container"" id=""loader-container"">
    ...
    class SelectTool {{
        ...
        render() {{
            ...
            this.data.options.forEach((option,index) => {{
                ...
                optionElement.value = option.value;
                optionElement.innerText = option.value;
                if (option.value === this.data.selected) {{
    ...
    class OnlyTextBlock {{
        ...
        render() {{
            ...
            this.inputElement.style.lineHeight = '1.2';
            this.inputElement.setAttribute('role', this.data.role);
            if (this.data.role !== userrole || this.data.value) {{
                this.inputElement.disabled = true;
            }}
    ...
    class TextAreaBlock {{
        ...
        render() {{
            ...
            if (this.data.role !== userrole || this.data.value.trim()) {{
                this.inputElement.disabled = true;
            }}
